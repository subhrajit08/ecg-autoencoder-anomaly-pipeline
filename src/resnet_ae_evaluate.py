...
import mlflow
from resnet_ae import ResNetAE, LATENT_DIM
...
def get_reconstruction_errors(model, test_X, batch_size=256):
    errors = []
    model.eval()
...
def evaluate():
    ...
    thresholds = np.percentile(errors, np.arange(50, 99, 0.5))
    best_f2, best_threshold = 0, 0

    for t in thresholds:
        preds = (errors >= t).astype(int)
        f2 = fbeta_score(test_y, preds, beta=1.0, zero_division=0)
        if f2 > best_f2:
            best_f2 = f2
            best_threshold = t
    ...
    with mlflow.start_run(run_name="resnet_eval", nested=True):
        ...
        mlflow.log_artifact(f'{OUTPUT_DIR}/resnet_evaluation.png')
...
def plot_results(errors, labels, threshold, auroc, auprc, f2, precision, recall, cm):
    ...
    fig.suptitle('ResNet-AE ECG Anomaly Detection — Evaluation Results', color='white', fontsize=13)
    colors = {'normal': '#00d4aa', 'anomaly': '#ff4d6d', 'line': '#f4a261'}

    ax = axes[0][0]
    ax.set_facecolor('#1a1a2e')
    ax.hist(errors[labels==0], bins=80, alpha=0.7, color=colors['normal'],  label='Normal',  density=True)
    ax.hist(errors[labels==1], bins=80, alpha=0.7, color=colors['anomaly'], label='Anomaly', density=True)
    ax.axvline(threshold, color=colors['line'], linestyle='--', linewidth=1.5, label=f'Threshold: {threshold:.3f}')
    ax.set_title('Reconstruction Error Distribution', color='white')
    ...
    plt.savefig(f'{OUTPUT_DIR}/resnet_evaluation.png', dpi=150,
                bbox_inches='tight', facecolor='#0f0f0f')
    print(f"\nPlot saved to {OUTPUT_DIR}/resnet_evaluation.png")
